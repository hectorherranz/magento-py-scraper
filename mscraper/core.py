...
class WebCrawler(object):
    ...
    def scrape_request(self, body, url):
        lst = []
        ...
        for item in items:
            ...
            try:
                product_title = re.sub(' +', ' ', item.find("", lambda
                    x: x == 'product-name' or x == 'product-item-name').text.encode('utf-8').replace("\r", "").replace(
                    "\n", "").strip())
            except (TypeError, KeyError, AttributeError), e:
                product_title = ""
            try:
                product_price = re.sub(' +', ' ', item.find("", lambda x: x == 'price-box' or x == 'price').text.encode(
            ...
            print "{} is selling for {} with path {} at {}, img {}".format(product_title, product_price,
                                                                           product_path_category, product_url,
                                                                           product_img)
        ...
        if (path.exists('output.csv')):
            ...
            with open('output.csv', 'wb') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(
                    ["site", "product_title", "product_price", "product_path_category", "product_img", "product_url"])
                writer.writerows(lst)
